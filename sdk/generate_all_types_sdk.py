...
{% for sdk_type in sdk_types %}
  ...
  if (defined({{ arkui_cross_sdk_modules }}.{{ _sdk_type }})) {
    {{ _sdk_type }}s = {{ arkui_cross_sdk_modules }}. {{ _sdk_type }}
    group("{{ _sdk_type }}") {
      public_deps = []
...
foreach(os, sdk_systems) {
  {% for sdk_type in sdk_types %}
    ...
    {% endif %}
    package_info_file =
        "$arkui_cross_sdk_copy_dir/$os/{{ sdk_type }}/arkui-x.json"
    package_info = {}
    package_info = {
      path = "{{ sdk_type }}"
...
group("generated_arkui_cross_sdk") {
  public_deps = []
  {% for sdk_type in sdk_types %}
    {% set _sdk_type = sdk_type.replace('-', '_') %}
    if (defined({{ arkui_cross_sdk_modules }}.{{ _sdk_type }})) {
  ...
  {% endfor %}
  public_deps += {{ arkui_cross_sdk_modules }}.extras
}""",  # noqa E501
                        trim_blocks=True,
                        lstrip_blocks=True)

    contents = template.render(
        arkui_cross_sdk_modules=options.sdk_modules,
